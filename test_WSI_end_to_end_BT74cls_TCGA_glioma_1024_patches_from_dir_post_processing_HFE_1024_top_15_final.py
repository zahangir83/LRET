...
output_path_from_model = '/Users/malom/Desktop/zahangir/projects/computational_pathology/Large_scale_histopathology_analysis/experimental_logs/74CLS_Path_ResNet50_2GPUs_final/testing/TCGA_glioma_1024px_patches_ouptuts/'
output_path = '/Users/malom/Desktop/zahangir/projects/computational_pathology/Large_scale_histopathology_analysis/experimental_logs/74CLS_Path_ResNet50_2GPUs_final/testing/TCGA_glioma_1024px_patches_ouptuts_merged/'
...
for case_ID in directory_case_ids:
    ...
    numpy_files = [x for x in sorted(os.listdir(os.path.join(patche_output_dir,'outputs_numpy_logs/'))) if x[-12:] == '_IDs_all.npy']
    name = numpy_files[0] 
    patch_IDs = name.split('_IDs')[0]

    
    ids_path = os.path.join(os.path.join(patche_output_dir,'outputs_numpy_logs/'),patch_IDs+'_IDs_all.npy')
    ids = np.load(ids_path,allow_pickle='True')
    exd_ids = np.expand_dims(ids, axis=-1)
    
    class_call_path = os.path.join(os.path.join(patche_output_dir,'outputs_numpy_logs/'),patch_IDs+'_names_all.npy')
    classes_predicted = np.load(class_call_path,allow_pickle='True')
    exd_classes_predicted = np.expand_dims(classes_predicted, axis=-1)
    
    lineage_call_path = os.path.join(os.path.join(patche_output_dir,'outputs_numpy_logs/'),patch_IDs+'_lineage_all.npy')
    ...
    group_call_path = os.path.join(os.path.join(patche_output_dir,'outputs_numpy_logs/'),patch_IDs+'_lineage_all.npy')
    group_predicted = np.load(group_call_path,allow_pickle='True')
    exd_group_predicted = np.expand_dims(group_predicted, axis=-1)
    
    tvstn_call_path = os.path.join(os.path.join(patche_output_dir,'outputs_numpy_logs/'),patch_IDs+'_tumor_vs_not_tumor_all.npy')
    tvsnt_predicted = np.load(tvstn_call_path,allow_pickle='True')
    exd_tvsnt_predicted = np.expand_dims(tvsnt_predicted, axis=-1)
    
    conf_values_path = os.path.join(os.path.join(patche_output_dir,'outputs_numpy_logs/'),patch_IDs+'_confvs_all.npy')
    ...
    dlfv_path = os.path.join(os.path.join(patche_output_dir,'outputs_numpy_logs/'),patch_IDs+'_fvs_all.npy')
...
top_5_outputs_saving_path_csv = os.path.join(output_path,'merge_SA_prediction_Logs_top_15_accuracy.csv')
top_5_outputs_saving_path_npy = os.path.join(output_path,'merge_SA_prediction_Logs_top_15_accuracy.npy')
