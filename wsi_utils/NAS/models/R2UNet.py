...
def build_R2UNetE(input_shape,num_classes):
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(2e-4), loss='binary_crossentropy',metrics = ['acc', 'mse'])
...
def build_R2UNetED_DP_final(input_shape,num_classes):
   ...
   model.compile(optimizer=tf.keras.optimizers.Adam(2e-4),loss=dice_coef_loss,metrics = [dice_coef, 'acc', 'mse'])
    
   #model.compile(optimizer=tf.train.AdamOptimizer(3e-4), loss='binary_crossentropy',metrics = ['acc', 'mse'])
    
   return model 

def Dilated_R2UNetED_DP_final(input_shape,dilate,addition,num_classes):
   ...
   # zero block
   x = tf.keras.layers.Conv2D(32, (3, 3), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(inputs)    
   rcnn_bn0 = Rec_conv2d_bn(x, 32, 3, 3)    
   conv0_f = tf.keras.layers.add([x, rcnn_bn0])

    # downsampling zero block..
   pool0 = tf.keras.layers.MaxPooling2D((2, 2),data_format='channels_last')(conv0_f)
   conv_pool0 = tf.keras.layers.Conv2D(64, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(pool0)
   ...
   conv1_f = tf.keras.layers.add([conv_pool0, rcnn_bn1])
    ...
    # downsampling first block..
   pool1 = tf.keras.layers.MaxPooling2D((2, 2),data_format='channels_last')(conv1_f)
   conv_pool1 = tf.keras.layers.Conv2D(128, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(pool1)
   ...
   conv2_f = tf.keras.layers.add([conv_pool1, rcnn_bn2])
    ...
    # downsampling first block..
   pool2 = tf.keras.layers.MaxPooling2D((2, 2),data_format='channels_last')(conv2_f)
   conv_pool2 = tf.keras.layers.Conv2D(256, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(pool2)
   ...
   conv3_f = tf.keras.layers.add([conv_pool2, rcnn_bn3])
    ...
    # downsampling first block..
   pool3 = tf.keras.layers.MaxPooling2D((2, 2),data_format='channels_last')(conv3_f)
   conv_pool3 = tf.keras.layers.Conv2D(512, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(pool3)
   ...
   # fifth block .....
   pool4 = tf.keras.layers.MaxPooling2D((2, 2),data_format='channels_last')(conv4_f)
   conv_pool4 = tf.keras.layers.Conv2D(1024, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(pool4)
   ...
   if dilate == 1:       
       ...
       dilate1 = tf.keras.layers.Conv2D(176,3, activation='relu', padding='same', dilation_rate=1, kernel_initializer='he_normal')(conv5_f)
       b7 = tf.keras.layers.BatchNormalization()(dilate1)
       b7 = tf.keras.layers.Dropout(rate=0.3)(b7)
       dilate2 = tf.keras.layers.Conv2D(176,3, activation='relu', padding='same', dilation_rate=2, kernel_initializer='he_normal')(b7)
       b8 = tf.keras.layers.BatchNormalization()(dilate2)
       b8 = tf.keras.layers.Dropout(rate=0.3)(b8)
       dilate3 = tf.keras.layers.Conv2D(176,3, activation='relu', padding='same', dilation_rate=4, kernel_initializer='he_normal')(b8)
       b9 = tf.keras.layers.BatchNormalization()(dilate3)
       b9 = tf.keras.layers.Dropout(rate=0.3)(b9)
       dilate4 = tf.keras.layers.Conv2D(176,3, activation='relu', padding='same', dilation_rate=8, kernel_initializer='he_normal')(b9)
       b10 = tf.keras.layers.BatchNormalization()(dilate4)
       b10 = tf.keras.layers.Dropout(rate=0.3)(b10)
       dilate5 = tf.keras.layers.Conv2D(176,3, activation='relu', padding='same', dilation_rate=16, kernel_initializer='he_normal')(b10)
       b11 = tf.keras.layers.BatchNormalization()(dilate5)
       b11 = tf.keras.layers.Dropout(rate=0.3)(b11)
       dilate6 = tf.keras.layers.Conv2D(176,3, activation='relu', padding='same', dilation_rate=32, kernel_initializer='he_normal')(b11)
       if addition == 1:
           conv5_f = tf.keras.layers.add([dilate1, dilate2, dilate3, dilate4, dilate5, dilate6])
           #up3 = UpSampling2D((2, 2))(dilate_all_added)
       else:
           #up3 = UpSampling2D((2, 2))(dilate6)
           conv5_f = dilate6
    # Decoder...
   up6_1 = tf.keras.layers.Conv2DTranspose(512, (3, 3), strides=(2, 2), padding='same',kernel_initializer='he_normal',data_format='channels_last')(conv5_f)
   up6_concat = tf.keras.layers.concatenate([up6_1, conv4_f], axis=channel_axis)
   up6_concat = tf.keras.layers.Conv2D(512, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up6_concat)
   up6_recconv = Rec_conv2d_bn(up6_concat,512, 3, 3)
   up6_f = tf.keras.layers.add([up6_recconv, up6_concat])
   conv6 = tf.keras.layers.Conv2D(512, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up6_f)
   
   up7_1 = tf.keras.layers.Conv2DTranspose(256, (3, 3), strides=(2, 2), padding='same',kernel_initializer='he_normal',data_format='channels_last')(conv6)
   up7_concat = tf.keras.layers.concatenate([up7_1, conv3_f], axis=channel_axis)
   up7_concat = tf.keras.layers.Conv2D(256, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up7_concat)
   up7_recconv = Rec_conv2d_bn(up7_concat,256, 3, 3)
   up7_f = tf.keras.layers.add([up7_recconv, up7_concat])
   conv7 = tf.keras.layers.Conv2D(256, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up7_f)

   up8_1 =tf.keras.layers.Conv2DTranspose(128, (3, 3), strides=(2, 2), padding='same',kernel_initializer='he_normal',data_format='channels_last')(conv7)
   up8_concat = tf.keras.layers.concatenate([up8_1, conv2_f], axis=channel_axis)
   up8_concat = tf.keras.layers.Conv2D(128, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up8_concat)
   up8_recconv = Rec_conv2d_bn(up8_concat,128, 3, 3)
   up8_f = tf.keras.layers.add([up8_recconv, up8_concat])
   conv8 = tf.keras.layers.Conv2D(128, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up8_f)
 
   up9_1 = tf.keras.layers.Conv2DTranspose(64, (3, 3), strides=(2, 2), padding='same',kernel_initializer='he_normal',data_format='channels_last')(conv8)
   up9_concat = tf.keras.layers.concatenate([up9_1, conv1_f], axis=channel_axis)
   up9_concat = tf.keras.layers.Conv2D(64, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up9_concat)
   up9_recconv = Rec_conv2d_bn(up9_concat,64, 3, 3)
   up9_f = tf.keras.layers.add([up9_recconv, up9_concat])
   conv9 = tf.keras.layers.Conv2D(64, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up9_f)

   up10_1 = tf.keras.layers.Conv2DTranspose(32, (3, 3), strides=(2, 2), padding='same',kernel_initializer='he_normal',data_format='channels_last')(conv9)
   up10_concat = tf.keras.layers.concatenate([up10_1, conv0_f], axis=channel_axis)
   up10_concat = tf.keras.layers.Conv2D(32, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up10_concat)
   up10_recconv = Rec_conv2d_bn(up10_concat,32, 3, 3)
   up10_f = tf.keras.layers.add([up10_recconv, up10_concat])
   conv10 = tf.keras.layers.Conv2D(32, (1, 1), activation='relu', padding='same',kernel_initializer='he_normal',data_format='channels_last')(up10_f)
   
   if num_classes <= 2:
        conv10 = tf.keras.layers.Conv2D(1, (1, 1), activation='sigmoid',kernel_initializer='he_normal',data_format='channels_last')(conv10)
   else:
        conv10 = tf.keras.layers.Conv2D(num_classes,(1,1), activation='softmax')(conv10)
