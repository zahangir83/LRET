...
def read_images_and_masks_for_inflammation_seg(data_path, image_h, image_w):
    ...
    for image_name in images:
        ...
        img_mask_r = cv2.resize(img_mask, dsize=(image_h, image_w), interpolation=cv2.INTER_NEAREST)

        acc_imgs[i] = acc_img_r
        imgs[i] = img_r
        imgs_mask[i] = img_mask_r
...
def read_images_and_masks_for_fibrosis_seg(data_path, image_h, image_w):
