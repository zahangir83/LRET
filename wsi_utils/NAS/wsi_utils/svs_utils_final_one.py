...
def fill_tissue_holes(bw_img):
    ...
    # Dialated & erode the binary image to fillup...()
    bw_fill = binary_dilation(bw_fill, structure=ROI_kernel,iterations=4).astype(bw_fill.dtype)
    bw_fill = binary_erosion(bw_fill, structure=ROI_kernel,iterations=4).astype(bw_fill.dtype)
...
def extract_patches_from_binary_image_name3parts(full_img,patch_h,patch_w, img_name, patches_saving_dir):
    ...
    print('==============')
    print(img_name)
    number_of_sep = len(img_name.split('_')) - 1
    image_id = img_name.split('_')[0]
    img_name_part1 = img_name.split('_')[1]
    img_name_part2 = img_name.split('_')[2]
...
def extract_same_size_patches_from_wsi_final(svs_img_dir, patches_saving_dir, patch_size, patches_exist=False):
...
#     return os.path.dirname(patches_sub_dir)
...
def patches_to_binary_image_from_seg_class_steatosis_masks_medpace_final(patches_dir,patch_saving_dir):
    ...
    text_part_first  = name_wo_ext_part1.split('_')[0]#'_'+name_wo_ext_part1.split('_')[3]
    ## if you don't use the 4th part it will read only the seg masks......
    text_part_last  = name_wo_ext_part1.split('_')[2]+'_'+name_wo_ext_part1.split('_')[3]
...
def create_heatmaps_from_roi_images_from_class_plus_seg_mask_st(roi_imagelogs_dir,patch_saving_dir):
    ...
    for k in range(num_rois):
        ...
        fill_gray_mask_input = cv2.morphologyEx(gray_mask_input,cv2.MORPH_OPEN,kernel)
        roi_mask = 255.0*(fill_gray_mask_input[:,:] >= 125)  
        ...
        ######################## COMBINED THE SEG_MASK AND THE ACTUAL TISSUE MASKS<<<<<<<<<<<<<<<<<<<<<<
#        #pdb.set_trace()
        #pred_masks_seg = 255.0*(combined_seg_mask[:,:,:] >= 0.5)               
        reconstructed_combined_seg_morph = perform_dilation_operations(combined_seg_mask) 
        seg_roi_combined_with_morphology_mask_name = actual_wsi_name+'_'+str(idv_roi_id)+'_combined_wmorph_mask.jpg'
        ...
        #reconstructed_pred_seg_morph =cv2.imread(seg_mask_path) ## heatmap from the seg_class mask...
        #reconstructed_pred_seg_morph =cv2.imread(final_des_combined_mask) ## heatmap from the combined mask...
        reconstructed_pred_seg_morph =cv2.imread(final_des_combined_mask_morph) ## heatmap from the combined mask with morphologi...
        im_heatmap = create_heatmap(roi_image, reconstructed_pred_seg_morph, a1=.5, a2=.5)   ## CREATING HEATMAP .... WITH COMBINED MASKS.....
        #im_heatmap = create_heatmap(roi_image, roi_seg_mask_input, a1=.5, a2=.5)    ## CREATING HEATMAP .... WITH COMBINED MASKS.....
        ...
        #pixels_wo_steatosis = total_number_tissue_pixels - total_steatosis_pixels
        percentage_of_steatosis_cells = (total_steatosis_pixels/total_number_tissue_pixels)
