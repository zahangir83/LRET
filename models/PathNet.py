...
def VGG16(include_top=True, weights='vggface',
    ...
    if input_tensor is not None:
        ...
        inputs = img_input
        # Create model.
    model = Model(inputs, x, name='Path_VGG16')  # load weights
   
    return model


def PathNet_VGG19P(include_top=True, 
          input_tensor=None, input_shape=None,
          pooling=None,
          classes=2622):
    input_shape = _obtain_input_shape(input_shape,
    ...
    if input_tensor is not None:
        ...
        inputs = img_input
        # Create model.
    model = Model(inputs, x, name='PathNet_VGG16')  # load weights
...
def PathNet_RESNET50(include_top=True, 
             input_tensor=None, input_shape=None,
             pooling=None,
             classes=8631):
    input_shape = _obtain_input_shape(input_shape,
    ...
    x = Conv2D(
        8, (7, 7), use_bias=False, strides=(2, 2), padding='same',
        name='conv1/7x7_s2')(img_input)
    x = BatchNormalization(axis=bn_axis, name='conv1/7x7_s2/bn')(x)
    x = Activation('relu')(x)
    ...
    # Block 2
    x = Conv2D(16, (3, 3), activation='relu', padding='same', name='conv2_1/3x3_b2')(
        x)
    x = Conv2D(16, (3, 3), activation='relu', padding='same', name='conv2_2/3x3_b2')(
        x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='pool2')(x)

    # Block 3
    x = Conv2D(32, (3, 3), activation='relu', padding='same', name='conv3_1/3x3_b3')(
        x)
    x = Conv2D(32, (3, 3), activation='relu', padding='same', name='conv3_2/3x3_b3')(
        x)
    x = Conv2D(32, (3, 3), activation='relu', padding='same', name='conv3_3/3x3_b3')(
        x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='pool3')(x)
...
def PathNet_SENET50(include_top=True, 
            input_tensor=None, input_shape=None,
            pooling=None,
            classes=8631):
    input_shape = _obtain_input_shape(input_shape,
    ...
    if input_tensor is not None:
        ...
        inputs = img_input
    # Create model.
    model = Model(inputs, x, name='PathNet_senet50')
