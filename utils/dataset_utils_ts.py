...
def extract_image_patches_hard_FP_metastatic(full_img,gt_mask,full_mask,patch_h,patch_w, img_name, imd_saving_dir):
    ...
    for r_s in range(rows):
        for c_s in range(columns):
            patch_img = full_img[r_s*patch_h: r_s*patch_h+patch_h,c_s*patch_w:c_s*patch_w+patch_w,:]
            patch_mask = full_mask[r_s*patch_h: r_s*patch_h+patch_h,c_s*patch_w:c_s*patch_w+patch_w]
            gt_mask = gt_mask[r_s*patch_h: r_s*patch_h+patch_h,c_s*patch_w:c_s*patch_w+patch_w]
            ...
            #mx_val_fp = patch_mask.max()
            num_white_fp = np.sum(patch_mask == 255)
            num_white_gt = np.sum(gt_mask == 255)
            print ('Summation of FP  : '+str(num_white_fp))
            print ('Summation of GT pixels  : '+str(num_white_gt))          

            if num_white_gt == 0 and num_white_fp > 0:
                cv2.imwrite(final_des_img,patch_img)
                #cv2.imwrite(final_des_mask,patch_mask)
            pn+=1
...
def save_false_true_positive_patches_from_prediction_mask_metastatic(image,gt_mask,pred_mask, patch_w, patch_h,image_name,false_positive_saving_path):
        
    # create gaussian mask...
    gt_mask = 255.0*(gt_mask[:,:]> 0)
    pred_mask = 255.0*(pred_mask[:,:]> 0)

    gt_not_mask = cv2.bitwise_not(gt_mask, mask = None) 
    mask_for_false_positive = cv2.bitwise_and(gt_not_mask, pred_mask, mask = None)
    
    #final_des_img = os.path.join(false_positive_saving_path,'test.png')
    #cv2.imwrite(final_des_img,mask_for_false_positive)
    #pdb.set_trace()   
    #extract_image_patches(image,mask_for_false_positive,patch_w, patch_h,image_name, false_positive_saving_path)
    
    extract_image_patches_hard_FP_metastatic(image,gt_mask, mask_for_false_positive,patch_w, patch_h,image_name, false_positive_saving_path)
    
    return mask_for_false_positive
