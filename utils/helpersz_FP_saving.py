...
def extract_9patches_from_image(img_4_patches,cntr_points_x,cntr_points_y, mitosis_height, mitosis_width,stride_pixels):
    ...
    patches_9[1,:,:,:] = cv2.resize(img_4_patches[xmin1:xmax1,ymin1:ymax1], dsize=(mitosis_height, mitosis_width), interpolation=cv2.INTER_LANCZOS4) 
    ...
    patches_9[2,:,:,:] = cv2.resize(img_4_patches[xmin2:xmax2,ymin2:ymax2], dsize=(mitosis_height, mitosis_width), interpolation=cv2.INTER_LANCZOS4)
    ...
    patches_9[3,:,:,:] = cv2.resize(img_4_patches[xmin3:xmax3,ymin3:ymax3], dsize=(mitosis_height, mitosis_width), interpolation=cv2.INTER_LANCZOS4)
    ...
    patches_9[4,:,:,:] = cv2.resize(img_4_patches[xmin4:xmax4,ymin4:ymax4], dsize=(mitosis_height, mitosis_width), interpolation=cv2.INTER_LANCZOS4)
    ...
    patches_9[5,:,:,:] = cv2.resize(img_4_patches[xmin5:xmax5,ymin5:ymax5], dsize=(mitosis_height, mitosis_width), interpolation=cv2.INTER_LANCZOS4)
    ...
    patches_9[6,:,:,:] = cv2.resize(img_4_patches[xmin6:xmax6,ymin6:ymax6], dsize=(mitosis_height, mitosis_width), interpolation=cv2.INTER_LANCZOS4)
    ...
    patches_9[7,:,:,:] = cv2.resize(img_4_patches[xmin7:xmax7,ymin7:ymax7], dsize=(mitosis_height, mitosis_width), interpolation=cv2.INTER_LANCZOS4)
    ...
    patches_9[8,:,:,:] = cv2.resize(img_4_patches[xmin8:xmax8,ymin8:ymax8], dsize=(mitosis_height, mitosis_width), interpolation=cv2.INTER_LANCZOS4)
...
def WTA_for_confidences(convf_vec):
    
    mean = convf_vec.mean(1)
    
    counter_h = 0
    counter_l = 0
    for k in range(len(convf_vec)):
...
def conf_vec_to_matrix_to_vec_for_9patches(conf_vec):
    ...
    for i in range(matrix_length):
        conf_matrix[i,:] = conf_vec[i*num_of_patches:(i*num_of_patches+num_of_patches)]  # orignal _one...   
    # select maximum values per row...finral conf. vector..    
    #max_confv_vec = conf_matrix.max(1)      
    # calculate the mean confident vector...
    mean_confv_vec = conf_matrix.mean(1)
    #std_confv_vec = conf_matrix.mean(1)
     
    return mean_confv_vec

def conf_vec_to_matrix_for_9patches(conf_vec):
...
def ensemble_conf_matrix_for_9patches(conv_matrix_2,conv_matrix_4,conv_matrix_6,conv_matrix_8):
    
    
    ensb_conf_concate_matrix = np.concatenate((conv_matrix_2,conv_matrix_4,conv_matrix_6,conv_matrix_8),axis=1)
    
    ensb_sorted_conf_concate_matrix = []
    ensb_highest_conf_matrix = []
    
    for k in range(len(ensb_conf_concate_matrix)):
        row_conf_vector = ensb_conf_concate_matrix[k,:]
        row_conf_vector.sort()
        ensb_sorted_conf_concate_matrix.append(np.array(row_conf_vector))
        ensb_highest_conf_matrix.append(np.array(row_conf_vector[-9:]))
    
    ensb_sorted_conf_concate_matrix = np.array(ensb_sorted_conf_concate_matrix)
    ensb_highest_conf_matrix = np.array(ensb_highest_conf_matrix)
    
    return ensb_conf_concate_matrix,ensb_sorted_conf_concate_matrix,ensb_highest_conf_matrix
    
    
def confidence_analysis_9patches_second_level(mitosis_x,mitosis_y,conf_vec,patch_level_threshold):
    ...
    for i in range(matrix_length):
        conf_matrix[i,:] = conf_vec[i*num_of_patches:(i*num_of_patches+num_of_patches)]  # orignal _one...   
    # select maximum values per row...finral conf. vector..
    # count how many time bigger than...0.9         
    max_confv_vec = conf_matrix.max(1)  
    mean_confv_vec = conf_matrix.mean(1)
    mtx_size = (len(max_confv_vec),1)
    #confgtp9_flag = []
    countergtp9 = []    
    v1_mitosis_x = []
    v1_mitosis_y = []  
    ...
    for k in range(len(max_confv_vec)):
        conf_vector = conf_matrix[k,:]
        counter = np.count_nonzero(conf_vector > patch_level_threshold) 
        sort_conf_vector =  conf_vector
        
        mean_val = np.array(conf_vector).mean()  
        max_val = np.array(conf_vector).max() 
        
        #pdb.set_trace()
        
        if mean_val > 0.1:
            sort_conf_vector.sort()
            conf_values = sort_conf_vector[-1]
            v1_mitosis_x.append(mitosis_x[k])
            v1_mitosis_y.append(mitosis_y[k])
            confidence_matrix.append(conf_vector)
            confidence_vec.append(conf_values)
            countergtp9.append(counter)
        else:
            if mean_val >= 0.01 and counter > 0 and max_val > 0.60 :
                sort_conf_vector.sort()
                conf_values = sort_conf_vector[-1]
                v1_mitosis_x.append(mitosis_x[k])
                v1_mitosis_y.append(mitosis_y[k])
                ...
                if mean_val >= 0.005 and counter > 0 and max_val > 0.75 :                    
                    sort_conf_vector.sort()
                    conf_values = sort_conf_vector[-1]
                    v1_mitosis_x.append(mitosis_x[k])
                    v1_mitosis_y.append(mitosis_y[k])
        ...
        #mean_confidence_val.append(mean_val)    

    # calculate the mean confident vector...
    #mean_confv_vec = np.array(conf_matrix).mean(1)
    #std_confv_vec = conf_matrix.mean(1)
    
    #pdb.set_trace()
        
    return v1_mitosis_x,v1_mitosis_y,confidence_vec,countergtp9,mean_confv_vec
...
def image_with_confidence(img_4_conf,cntr_points_x,cntr_points_y,conf_vec,threshold):
    ...
    for x,y,c in zip(cntr_points_x,cntr_points_y,conf_vec):  
        ...
        if c>threshold:            
            ...
            c_f =round(c,5)
            cv2.putText(img_4_conf,str(c_f),(int(x)-13,int(y)), font,0.3,(0,255,255))        
            #cv2.rectangle(img_4_conf,(x-15,y-15),(x+15,y+15),(0,255,0),thickness)
            # Radius of circle 
            radius = 20
            # Blue color in BGR 
            color = (0, 255, 0)         
            ...
            cv2.circle(img_4_conf, (x, y), radius, color, thickness) 
        index = index + 1            
    cv2.putText(img_4_conf,'Total : '+str(total_mitosis),(int(30),int(30)), font,1,(255,0,0))         
    return img_4_conf,total_mitosis,m_points_x, m_points_y,m_indexs,m_confident

def foregournd_mitosis_regions(image,cntr_points_x,cntr_points_y,conf_vec,threshold):
    
    height, width, channels = image.shape   
    
    foreground_mitosis_region_image = np.zeros((height, width, channels),dtype=np.uint8)
    ...
    for x,y,c in zip(cntr_points_x,cntr_points_y,conf_vec):  
        ...
        if c>threshold:            
            ...
            m_points_y.append(y)
            m_confident.append(c)
            foreground_mitosis_region_image[y-32:y+32,x-32:x+32,:] = image[y-32:y+32,x-32:x+32,:]
    
    return foreground_mitosis_region_image,total_mitosis,m_points_x, m_points_y
...
def draw_rectangle_for_gt(img_4_conf,cntr_points_x,cntr_points_y):
    ...
    for x,y in zip(cntr_points_x,cntr_points_y):             
        ...
        #cv2.rectangle(img_4_conf,(x-15,y-15),(x+15,y+15),(255,0,0),thickness)
        #cv2.circle(img_4_conf, (x, y), 2, (0,0,255), -1)
        # Radius of circle 
        radius = 22
        # Blue color in BGR 
